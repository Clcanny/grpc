...
void create_sockets(int sv[2]) {
  ...
  flags = grpc_socket_factory_fcntl(sv[0], F_GETFL, 0);
  CHECK_EQ(grpc_socket_factory_fcntl(sv[0], F_SETFL, flags | O_NONBLOCK), 0);
  flags = grpc_socket_factory_fcntl(sv[1], F_GETFL, 0);
  CHECK_EQ(grpc_socket_factory_fcntl(sv[1], F_SETFL, flags | O_NONBLOCK), 0);
